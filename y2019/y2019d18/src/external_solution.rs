...
fn search(graph: HashMap<char, HashMap<char, usize>>, start: char) -> usize {
    ...
    while let Some(current) = priority_queue.pop() {
        if current.keys.len() == key_count {
            // println!("execute_search_count: {}", counter);
        ...
        //     .entry(cache_key)
        //     .or_insert_with(|| {
        //         execute_search_count += 1;
        //         search_keys(&graph, &current.keys, current.node)
        //     });

        let cached_entry = search_keys(&graph, &current.keys, current.node);
        // println!("{} from {}, found {}, queue {}", counter, current.node, cached_entry.len(), priority_queue.len());
        // if counter == 9 {
        //     println!()
        // }
        counter += 1;
